...
# A list of all algorithms to be tested
ALGORITHMS_TO_TEST = ["bubble", "selection", "quick", "merge"]

@pytest.mark.parametrize("algo", ALGORITHMS_TO_TEST)
@pytest.mark.parametrize("input_list, expected_sorted_asc", TEST_CASES)
def test_all_algorithms(sorter, algo, input_list, expected_sorted_asc):
    """
    ...
    """
    # --- Test Ascending ---
    result_asc = sorter.sort(input_list, algo, ascending=True)
    ...
    result_desc = sorter.sort(input_list, algo, ascending=False)
    assert result_desc == expected_sorted_desc, \
        f"{algo} sort (descending) failed for case: {input_list}"

@pytest.mark.parametrize("algo", ALGORITHMS_TO_TEST)
def test_large_random_list(sorter, algo):
...
def test_original_list_is_not_modified(sorter):
